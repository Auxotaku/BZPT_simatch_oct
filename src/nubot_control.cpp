...
namespace nubot
{
    ...
    public:
        ...
        void goal_keeper()
        {
            ...
            if (ball_is_free() && /*robot_pos_.distance(ball_pos_) <= 55*/ world_model_info_.field_info_.isOurGoal(ball_pos_))
            {
                action_cmd_.handle_enable = 1;
                action_cmd_.move_action = CatchBall;
                action_cmd_.rotate_acton = CatchBall;
        ...
        void normalGame()
        {
            ...
            if (world_model_info_.AgentID_ == 1)
            {
                ...
                //处理回传球
                if(world_model_info_.pass_state_.catch_id_ == 1)
                {
                    m_plan_.CatchPassedBall();
                }
                //处理守门员持球
                else if(world_model_info_.RobotInfo_[0].getDribbleState())
                {
                    //传出
                    for(int i = 2; i <= 5; ++i)
                    {
                        int mode = m_plan_.canPass(i);
                        if(mode)
                        {
                            m_plan_.PassBall_Action(i, mode);
                            break;
                        }
                    }
                }
